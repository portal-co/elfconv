...
void __svc_native_call(void) {
  ...
  switch (state_gpr.x8.qword) {
    ...
    case AARCH64_SYS_GETRUSAGE: /* getrusage(struct rusage *u) */
    {
      rusage _rusage;
      int ret = getrusage(state_gpr.x0.dword, &_rusage);
      memcpy(_ecv_translate_ptr(state_gpr.x1.qword), &_rusage, sizeof(rusage));
      state_gpr.x0.dword = ret;
    } break;
    ...
    case AARCH64_SYS_MPROTECT: /* mprotect (unsigned long start, size_t len, unsigned long prot) */
      ...
      break;
    case AARCH64_SYS_WAIT4: /* wait4(pid_t pid, int *stat_addr, int options, struct rusage *u) */
      state_gpr.x0.dword = wait4(state_gpr.x0.dword, (int *) _ecv_translate_ptr(state_gpr.x1.qword),
                                 state_gpr.x2.dword, (rusage *) state_gpr.x3.qword);
      break;
